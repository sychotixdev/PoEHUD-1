...
namespace PoeHUD.Poe.Elements
{
    public class StashElement : Element
    {
        ...
        private Element ViewAllStashPanel => Address != 0 ? GetObject<Element>(M.ReadLong(Address + 0xA48, 0x10, 0xb48)) : null;
        ...
        public string getStashName(int number)
        {
            if (number >= TotalStashes || number < 0)
                return string.Empty;
            long add = ViewAllStashPanel.Children[2].Children[number].Address;
